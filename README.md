...
这次优化不是把 OCR 文本直接交给大模型，而是补齐从页面公式到最终回答的完整链路：

| 原有问题 | 具体优化 | 产生的效果 |
|---|---|---|
| DeepDoc 能检测公式区域，但复杂公式可能只剩扁平 OCR | 以“公式编号 + 公式裁剪图”为视觉转写边界，调用 RAGFlow 租户已配置的 GLM Vision 逐符号读取 | 分式、上下标、求和范围、偏导、矩阵和大括号可以恢复为结构化 LaTeX |
| SymbolMT/MathType 字符被解析成 `U+F0xx` 私有区乱码 | 增加显式 SymbolMT 字符映射，只转换可确认的数学符号 | 希腊字母、关系符、积分与求和符不再显示成方框乱码 |
| 正则补公式容易悄悄改变数学含义 | 只自动修复可证明安全的重复 OCR 和尾部花括号；其余必须走公式图片识别 | 避免把错误但“看起来像公式”的内容写入知识库 |
| 公式 Chunk 只有公式文本，没有语义名称 | 写入公式名称、编号、来源文档、PDF 页码和邻近原文，并生成 `questions/important_keywords` | 可以用“单孤子解”“色散关系”或“式 (2.4)”等自然语言找回公式 |
| 正文提到某个式号时可能压过公式本体 | 检索扩展后按权威 `\tag{编号}` 提升公式本体，普通上下文引用不作为公式身份 | 按编号提问时优先返回真正含该公式的 Chunk |
| 修改 Chunk 后旧向量仍对应旧 OCR | 通过 RAGFlow Chunk PATCH API 写回，重新分词、Embedding 和索引 | 新名称、新问题和 LaTeX 都能参与混合检索 |
| 生成模型可能只回答“原文给出了表达式” | 公式问题的生成约束要求先输出名称、编号和完整 LaTeX，再解释符号 | 用户答案中直接呈现具体公式，而不是只有说明性文字 |
| 文档管理页只能看到原始字符串 | Chunk 接口增加 `formula_format/formula_name/equation_numbers`，聊天、来源和 Chunk 弹窗统一使用 KaTeX | 管理端能核对公式元数据，用户端能看到排版后的公式 |

当前论文样本的实测审计结果：扫描 99 个 Chunk，识别出 27 个唯一编号公式，
27 个均已写成标准 LaTeX，`needs_visual_review=0`，未发现花括号不平衡；参数说明
等普通文本不会被误判为公式。真实问答调用已验证可直接返回 (2+1) 维 Ito 方程
N-孤子解式 (1.1) 及其色散关系式 (1.2)。这些数字是当前样本文档的回归结果，
不是对所有 PDF 的通用准确率声明。
